...
public class Controller {
    ...
    // major
    @FXML private CheckBox compSciCheck;
    @FXML private CheckBox businessAnalystCheck;
	...
	public void initialize() {
		...
		compSciCheck.setOnAction(event -> {
			businessAnalystCheck.setSelected(false);
			courseComboBox.getItems().clear();
			courseComboBox.getItems().addAll(
					"Python", "Swift", "PHP", "Database"
			);
		});
		
		businessAnalystCheck.setOnAction(event -> {
			compSciCheck.setSelected(false);
			courseComboBox.getItems().clear();
			courseComboBox.getItems().addAll(
					"Process Management", "Supply Chain", "Resource Planning"
			);
		});
		...
		displayButton.setOnAction(event -> {
			...
			stringBuilder.append("Email: " + emailField.getText() + "\n");
			stringBuilder.append("Activies: " + checkBoxes.stream().filter(CheckBox::isSelected)
					.map(checkbox -> checkbox.getText()).collect(Collectors.joining(", ")) + "\n");
			stringBuilder.append("Course: " + courseComboBox.getSelectionModel().getSelectedItem());
